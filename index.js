...
bot.on('guildBanAdd', member => {
  ...
  .setAuthor(bot.user.username, bot.user.avatarURL)
  .setThumbnail(member.user.avatarURL)
  .setTitle(`${member.user.username} Sunucudan yasaklandı.`)
  .setTimestamp()
  channel.sendEmbed(embed); 
});
...
bot.on('guildBanRemove', member => {
  ...
  .setAuthor(bot.user.username, bot.user.avatarURL)
  .setThumbnail(member.user.avatarURL)
  .setTitle(`${member.user.username} Sunucudan yasaklanma kaldırıldı.`)
  .setTimestamp()
  channel.sendEmbed(embed); 
});
...
bot.on('messageDelete', msg => {
  ...
  .setAuthor(bot.user.username, bot.user.avatarURL)
  .setThumbnail(member.user.avatarURL)
  .setTitle(`${member.user.username} tarafından <#${msg.channel.id}> kanalına gönderilen "${member.content}" mesajı silindi.`)
  .setTimestamp()
  channel.sendEmbed(embed); 
});
...
bot.on('channelCreate', member => {
  ...
  const embed = new Discord.RichEmbed()
  .setColor('0xff1a1a')
  .setTitle(`<#${channel.id}> kanalı oluşturuldu.`)
  .setTimestamp()
  channel.sendEmbed(embed); 
});

bot.on('channelDelete', member => {
  const channel = member.guild.channels.find('name', 'modlog');
  if (!channel) return;
  const embed = new Discord.RichEmbed()
  .setColor('0xff1a1a')
  .setTitle(`<#${channel.name}> kanalı silindi.`)
  .setTimestamp()
  channel.sendEmbed(embed); 
});

bot.on('messageUpdate', member => {
  const channel = member.guild.channels.find('name', 'modlog');
  if (!channel) return;
  const embed = new Discord.RichEmbed()
  .setColor('0xff1a1a')
  .setTitle(`${oldMsg.author} adlı kullanıcı <#${oldMsg.channel.id}> kanalına gönderdiği "${oldMsg.content}" mesajını "${newMsg.content}" olarak düzenledi.`)
  .setTimestamp()
  channel.sendEmbed(embed); 
});
