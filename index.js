...
.setColor("RANDOM"));
	...
	bot.on('messageDelete', async msg => {
			...
			.setColor(15158332)
			.setDescription(`<@!${msg.author.id}> tarafından <#${msg.channel.id}> kanalına gönderilen "${msg.content}" mesaj silindi.`)
			.setFooter(`ID: ${msg.id}`)
	...
	bot.on('channelCreate', async channel => {
			...
			if (channel.type === "voice") {
				...
				.setColor(3066993)
				.setAuthor(channel.guild.name, channel.guild.iconURL)
				.setDescription(`${channel.name} sesli kanalı oluşturuldu.`)
				.setFooter(`ID: ${channel.id}`)
		         	trtr.sendEmbed(embed);
			}
		}
